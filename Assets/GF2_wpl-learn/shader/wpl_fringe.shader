Shader "URP/wpl_fringe"
{
    Properties
    {
        ...
        _AnisotropicSlide			("AnisotropicSlide", Range(-1, 1))	= 0.3
        ...
        [Header(FringeInnerOuterStencil)]
        ...
        [Header(FringeShadowCaster(Stencil))]
        //_Color ("Color", Color) = (1, 1, 1, 1)
        //_Offset ("Offset", float) = 0.02
        _FriStencil("Stencil ID", Float) = 0
        ...
        _HighLightMask("HighLightMask",2D) = "black"{}
        _SpecularShiftTex("_SpecularShift",2D) = "black"{}
        _SpecularWidth("_SpecularWidth",Float) = 0.5
        _SpecularScale("_SpecularScale",Float) = 0.5
        _SpecularShiftValue("SpecularShift",Float) = 0.5
        ...
        Pass
        {
            ...
            Stencil 
           {
            ...
            half4 ToonFrag(Toon_v2f i) : SV_Target
            {
                ...
                half3 hairSpecColor = hairSpecTex * _SpecColor * hairSpecStrength * 2;
                ...
                * mainLight.color * mainLight.shadowAttenuation * directOcclusion;
                ...
                float3 RimColor = 0;
                #if _Rim_Key
                    //偏移UV
                    float2 RimOffsetUV = screenUV + viewNormal.xy * _RimOffset * 0.01;
                    //采样深度图
                    ...
                    float diff = linear01EyeOffestDepth - linear01EyeTrueDepth;    //深度差
                    float rimMask = step(0.1, diff);
                    RimColor = rimMask * addLightResult * _EmissionIntensity;
                #endif

               

                half3 lightingResult = directLightResult + indirectLightResult + 0.3 * addLightResult + emissResult + RimColor;
                lightingResult = saturate(lightingResult) + hairSpecColor;

                float3 basecolor = SAMPLE_TEXTURE2D(_BaseMap,sampler_BaseMap,i.uv.xy);
                basecolor += hairSpecColor;
