...
#ifndef GF2_TOON_INCLUDED
#define GF2_TOON_INCLUDED
...
/*
 * D: Trowbridge-Reitz GGX 就是普遍使用的D_GGX_UE4
...
// sigmoid sss feature
float sigmoid(float x, float offest, float sharp) 
{
    float s;
    s = 1 / (1 + pow(100000, (-3 * sharp * (x - offest))));
    return s;
};
...
float3 IndirectDiffuse(float3 normalWS, half upDirLerp, half4 selfEnvColor, half3 albedo, float3 F0, float NdotV, float roughness, float metallic, float occlusion)
{
	...
	float3 SHColor = SampleSH(SHNormal);//球谐光照颜色
...
// 采样渐变阴影
half4 SampleDirectShadowRamp(TEXTURE2D_PARAM(RampTex, RampSampler), float lightRange, float V = 0.125)
{
    float2 shadowRampUV = float2(lightRange, V);
...
real3 FragLigthingPBR(Light mainLight, MY_PBR_DATA pbrData, out half3 dirResult, out half3 indirResult)
{
    ...
    float D = DistributionGGX(NdotH, roughnessSquare);
...
struct Toon_v2f 
{
    ...
    float eyesmask          :TEXCOORD6;
	UNITY_VERTEX_INPUT_INSTANCE_ID
};

Toon_v2f ToonVert(Toon_a2v v)
{
	Toon_v2f o;
    ...
    o.eyesmask = lerp(step(0,v.vertex.x),step(v.vertex,0),isLeftSide);
    o.eyesmask = o.eyesmask * 0.2 + 0.8;
    float2 faceFrontXZ = half2(0,1);
    half2 dotFaceCamera = dot(faceFrontXZ,camFrontXZ);

    /*float faceFrontYZ = float2(0,1);
    float2 cameFrontYZ = -UNITY_MATRIX_V[2].yz;
    float isupSide = step(0.92,dot(cameFrontYZ,faceFrontXZ));*/
    o.eyesmask = o.eyesmask * dotFaceCamera;
	return o;
}
